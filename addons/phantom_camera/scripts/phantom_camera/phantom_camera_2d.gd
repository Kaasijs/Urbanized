...
@export var auto_zoom_speed:float = 10.0
			...
			if _has_multiple_follow_targets:
				...
				if auto_zoom:
					...
					if rect.size.x > rect.size.y * _phantom_camera_manager.screen_size.aspect():
						zoom = lerp(zoom, clamp(_phantom_camera_manager.screen_size.x / rect.size.x, auto_zoom_min, auto_zoom_max) * Vector2.ONE , delta*auto_zoom_speed)
					else:
						zoom = lerp(zoom, clamp(_phantom_camera_manager.screen_size.y / rect.size.y, auto_zoom_min, auto_zoom_max) * Vector2.ONE , delta*auto_zoom_speed)
